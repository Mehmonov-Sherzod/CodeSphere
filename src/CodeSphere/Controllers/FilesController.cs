...
namespace CodeSphere.Controllers
{
    ...
    public class FilesController : ControllerBase
    {
        ...
        public FilesController(IFileStoreageService fileStorageService)
        {
        ...
        {
            Console.WriteLine($"[Files] Download request - Bucket: '{bucketName}', Object: '{objectName}'");
            ...
            try
            {
                Console.WriteLine($"[Files] Calling DownloadFileAsync...");
                var stream = await _fileStoreageService.DownloadFileAsync(bucketName, objectName);
                Console.WriteLine($"[Files] Download successful, stream length: {stream.Length}");
        ...
        public async Task<IActionResult> DeleteFile([FromQuery] string bucketName, [FromQuery] string objectName)
        {
            ...
            try
            {
                bool removed = await _fileStoreageService.RemoveFileAsync(bucketName, objectName);
                if (removed)
                {
